...
int BlockCache::read(uint64_t offset, size_t len, uint8_t* out_buffer) {
    std::lock_guard<std::mutex> lock(mutex_);
    
    if (id_to_slot_.empty()) {
        return -1; // Cache not initialized or empty
    }
    ...
    for (uint64_t b = start_block; b <= end_block; ++b) {
        ...
        // Calculate length to copy from this block
